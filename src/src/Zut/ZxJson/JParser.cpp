...
{
  JParser::JParser(const std::span<const char> spJson)
      : m_spJson{ spJson }
  {
    // skip utf-8 bom
    if (spJson[0] == '\xEF' && spJson[1] == '\xBB' && spJson[2] == '\xBF')
    {
      this->AddReadBytes(3);
    }
  }
  ...
  auto JParser::SkipWhite() -> char
  {
    while (true)
    {
      ...
      case '\n':
      case '\r':
      case ' ': this->AddReadBytes(); continue;
      }
      return tmp_char;
    }
  }

  auto JParser::NextToken() -> char // NOLINT
  {
    ...
    case '8':
    case '9':
    case '-': return tmp_char;
    case ',': this->AddReadBytes(); return this->NextToken();
  ...
  auto JParser::ParseArray(JValue& rfJValue) -> void // NOLINT
  {
  ...
  auto JParser::ParseObject(JValue& rfJValue) -> void // NOLINT
  {
    assert(this->CurToken() == '{');
  ...
  auto JParser::ParseNumber(JValue& rfJValue) -> void
  {
    char* end{};
    const char* beg = this->CurPtr();
    const auto num_org = std::strtod(beg, &end);
    ...
    auto fn_decode_unicode = [this](std::string& rfStr)
    {
      auto fn_read_code_point = [this]() -> uint16_t
      {
        auto fn_char2num = [](char cChar) -> uint8_t
        {
          ...
          if (char_v >= 0x61 && char_v <= 0x7a)
          {
            return char_v - 0x57;
          } // a-z
          else if (char_v >= 0x41 && char_v <= 0x5A) // NOLINT
          {
            return char_v - 0x37;
          } // A-Z
          else if (char_v >= 0x30 && char_v <= 0x39)
          {
            return char_v - 0x30;
          } // 0-9
          else
          {
            throw std::runtime_error("ZxJson::JParser::ParseString(): read code point error!");
          }
        };
  ...
  auto JParser::ParseValue(JValue& rfJValue) -> void // NOLINT
  {
